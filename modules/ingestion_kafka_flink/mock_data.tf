...
data "aws_iam_policy_document" "mock_data_task_policy" {
  # Statement 1: Grant cluster-level permissions.
  # Statement 1: 授予集群级别的权限
  statement {
    effect = "Allow"
    actions = [
      "kafka-cluster:Connect",                 # Allow connection / 允许连接
      "kafka-cluster:DescribeCluster",         # Allow cluster description / 允许描述集群
      "kafka-cluster:WriteDataIdempotently"    # Allow idempotent writing / 允许幂等性写入 (防止消息重复)
    ]
    resources = [
  ...
  statement {
    effect = "Allow"
    actions = [
      "kafka-cluster:*Topic*",                 # Allow create, describe, modify topics / 允许创建、描述、修改 Topic  https://docs.aws.amazon.com/zh_cn/IAM/latest/UserGuide/reference_policies_elements_action.html
      "kafka-cluster:WriteData",               # Allow writing to topics / 允许向 Topic 写入数据
      "kafka-cluster:ReadData"                 # Some clients need read metadata / 某些客户端库在生产消息时也需要读取元数据
    ]
    resources = [
      # Authorize all topics
      # 授权操作所有 Topic
      "arn:aws:kafka:${var.aws_region}:${data.aws_caller_identity.me.account_id}:topic/${var.project_name}-${var.environment}-msk-cluster/*",
      # Authorize all transactional IDs
  ...
  # Statement 3: Retain original SSM parameter permissions.
  ...
  container_definitions = jsonencode([
    {
      ...
      linuxParameters = {
        # Configure Linux features like init process.
        # 用于配置容器的 Linux 特性，确保一些特定的进程管理功能（如 init 进程）正常工作。
        initProcessEnabled = true 
      },
      executeCommandConfiguration = {
        # Enable exec command for debugging.
        # 启用在容器内执行命令的功能，允许你通过 aws ecs execute-command 在容器中执行交互式命令进行调试和管理。
        enabled = true 
      }
      logConfiguration = {
        logDriver = "awslogs"
...
# ------------------------------------------------------------------------------
# Simple Answer: In Terraform (AWS Provider), `aws_cloudwatch_event_rule` IS EventBridge Rule. 
# 简单的回答：
...
#  1. History:
#  1. 历史原因 (History):
#      * Originally called CloudWatch Events. Terraform created `aws_cloudwatch_event_rule`.
#      * 最初，这个服务叫 CloudWatch Events。
#      * Terraform developed the aws_cloudwatch_event_rule resource to correspond to it at the time.
#      * Terraform 当时就开发了 aws_cloudwatch_event_rule 这个资源来对应它。
#      * AWS renamed it to EventBridge, but Terraform kept the name for compatibility.
...
#      * Terraform added `aws_cloudwatch_event_bus` for new features.
#      * Terraform 后来引入了 aws_cloudwatch_event_bus 等新资源来支持 EventBridge 的新特性（比如自定义总线）。
#      * But for basic Rules, `aws_cloudwatch_event_rule` is still standard.
#      * 但对于最基础的 Rule（规则）和 Target（目标），大家依然普遍使用 aws_cloudwatch_event_rule。
#      * Note: For scheduled tasks, EventBridge Scheduler (`aws_scheduler_schedule`) is newer, but for event patterns, use Rule.
...
# Conclusion:
# 结论：
# Even if you write 'resource "aws_cloudwatch_event_rule" ...' in Terraform code, look for it on the Amazon EventBridge -> Rules page in the AWS Console. They are the same thing.
...
resource "aws_cloudwatch_event_target" "mock_data_task_target" {
  count     = var.mock_data_schedule == null ? 0 : 1
  # When using count, resource becomes a list.
  ...
  ecs_target {
    ...
    network_configuration {
      ...
      assign_public_ip = false # Run in private network for security / 在私有网络中运行，更安全
    }
  }
}
