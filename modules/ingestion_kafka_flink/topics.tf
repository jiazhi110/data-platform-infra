...
# Grant current IAM identity permission to create topics in the Kafka cluster.
# 授予当前 IAM 身份在 Kafka 集群中创建 Topic 的权限
resource "kafka_acl" "terraform_topic_creator_acl" {
  # The User: prefix is the standard format for Kafka ACLs.
  # 这里的 User: 前缀是 Kafka ACL 的标准格式
...
resource "kafka_topic" "produce_events" {
  ...
  config = {
    ...
    # MinISR must be one less than RF to preserve high availability.
    # 修复 AWS 警告：即使一个副本不可用，也能继续写入 MinISR must be one less than RF to preserve high availability.
...
# 1. Relationship between Partitions and Brokers
# 1. 分区（Partition）与 Broker 的关系

# First, a Kafka topic is divided into multiple partitions. A partition is the basic unit of storage in Kafka. Each partition is an ordered message queue. Kafka writes messages to partitions in order and reads them based on the consumer's offset.
# 首先，Kafka 主题会被分为多个 分区（partition）。分区是 Kafka 中用于存储消息的基本单位。每个分区是一个有序的消息队列。Kafka 将消息按顺序写入分区，并根据消费者的消费进度来读取消息。

# Each partition can only have one leader broker handling read/write operations. Other brokers are called follower brokers; they only synchronize data and do not handle actual requests.
# 每个分区只能由一个 leader broker 来处理读写操作，其他的 Kafka broker 被称为 follower broker，它们只是同步数据而不会处理实际的读写请求。

# Role of Brokers:
# Broker 的作用：

# Leader Broker: Handles all message writes and reads for the partition.
# Leader Broker：每个分区都有一个主分区，这个主分区由一个 leader broker 负责处理所有的消息写入和读取。

# Follower Brokers: Act as backups storing partition data, synchronizing from the leader in real-time.
# Follower Brokers：其他的 follower broker 会作为备份存储分区的数据，实时同步 leader 的数据。

# 2. Relationship between Replication Factor and Partitions
# 2. 副本因子（Replication Factor）与 Partition 的关系

# Replication factor refers to the number of copies of each partition in Kafka. Replicas are distributed across different brokers to ensure high availability and fault tolerance.
# 副本因子（replication factor） 是指 Kafka 中每个分区的副本数。副本分布在不同的 Kafka broker 上，副本因子决定了每个分区的消息会有多少份副本被复制。副本的主要目的是为了保证 数据的高可用性和容错性。

# How it affects Partitions:
# 如何影响 Partition：

# Replicas: Each partition has one leader and multiple followers. The replication factor determines the total count.
...
# Replicas are distributed across brokers. If a broker fails, Kafka promotes a follower to leader.
# 这些副本被分布在不同的 broker 上。如果某个 broker 宕机，Kafka 会根据副本副本策略，将一个 follower 提升为 leader，以确保分区数据的可用性和完整性。

# Role of Partitions and Replicas:
# 分区和副本的作用：

# Data Redundancy: Copies are stored on different brokers. Even if one fails, others can serve the data.
# 每个分区的 数据副本 存储在不同的 broker 上。这样，如果一个 broker 因为硬件故障或网络问题无法访问，其他 broker 上的副本仍然能够继续服务，保证数据的高可用性。

# Normally, consumers read from the leader. Followers are for consistency and backup.
# 在正常情况下，消费者总是从 leader broker 获取数据，而 follower brokers 只是用来保持数据的一致性和备份。

# When the leader fails, Kafka automatically elects a new leader from followers.
# 当 leader broker 出现故障时，Kafka 会自动从 follower broker 中选择一个新的 leader 来接管，确保系统不宕机。

# 3. How to Ensure High Availability via Partitions and Replication
# 3. 如何通过 Partition 和 Replication 确保高可用性

# High availability is achieved through Partitions, Replicas, and the Leader/Follower model. Let's explain with an example:
# Kafka 的高可用性是通过 分区、副本 和 leader/follower 模型来实现的。让我们通过一个具体的例子来解释：

# Suppose you have a Kafka cluster with 3 brokers:
...
# You have a topic with 3 partitions and a replication factor of 3. This means:
# 你有一个主题（topic），它的 partition 数量为 3，replication factor 为 3，这意味着：

# Each partition has 3 replicas (1 leader, 2 followers) distributed across brokers.
...
# Data Redundancy: Multiple copies on different brokers ensure availability even during failures.
# 数据冗余：每个分区的数据会有多个副本，这些副本分布在不同的 broker 上。这意味着即使某个 broker 出现故障，其他 broker 上的副本仍然可以保证数据的可用性。

# Fault Recovery: If Broker A fails, Kafka automatically elects a new leader.
# 故障恢复：如果 Broker A 宕机，Kafka 会自动选举一个新的 leader：

# For Partition 1, Broker A was the leader. Now it's down, Kafka elects B or C as the new leader.
...
# Load Balancing: Distributing partitions across brokers balances the load.
# 负载均衡：通过将分区和副本分布在多个 broker 上，Kafka 可以平衡负载。如果某个 broker 被请求过多，系统可以将流量转移到其他 broker 上，提高整体的吞吐量和性能。

# 4. Impact of Replication Factor:
# 4. 副本因子的影响：

# RF=1: No fault tolerance. If the leader fails, data is lost.
# 副本因子设置为 1：每个分区只有一个副本（即只有 leader），如果 leader 宕机，数据会丢失，Kafka 集群不可用。这种配置适用于对可用性要求不高的场景，但它没有容错性。

# RF=2 or 3: Provides fault tolerance. RF=3 can tolerate 2 broker failures.
...
# Replication Factor: Number of copies per partition for high availability.
# Replication Factor：定义了每个分区的副本数，副本存储在不同的 broker 上。副本因子是确保数据高可用性和容错性的重要手段。

# High Availability: Achieved by distributing replicas across brokers.
# 高可用性：通过副本分布在不同的 broker 上，Kafka 确保即使部分 broker 宕机，数据依然能够通过其他 broker 上的副本继续访问。

# Leader/Follower: One leader handles IO, followers sync data and takeover on failure.
# Leader/Follower：每个分区都有一个 leader 和多个 follower，只有 leader 负责处理读写操作，而 follower 则同步数据并在 leader 宕机时接管其角色。

# Proper configuration ensures HA, fault tolerance, and scalability.
